...
def replace_urls(content: str, to_modrinth: bool) -> str:
    if to_modrinth:
        pattern = re.compile(r'https?://(?:www\.)?curseforge\.com/minecraft/(?:mc-mods|texture-packs)/[A-Za-z0-9_\-./%]+')
        def repl(m):
            start = m.start()
            end = m.end()
            ...
            matched = m.group(0)
            ml = matched.lower()
            if 'shields.io' in ml or 'img.shields' in ml:
                return matched
            if re.search(r'<a[^>]+href\s*=\s*["\']\s*' + re.escape(matched) + r'\s*["\'][^>]*>\s*<img[^>]+src\s*=\s*["\'][^"\']*shields[^"\']*["\']', ctx, flags=re.IGNORECASE):
                return matched
            if re.search(r'<img[^>]+src\s*=\s*["\']\s*' + re.escape(matched) + r'\s*["\']', ctx, flags=re.IGNORECASE):
                return matched
            if re.search(r'!\[[^\]]*\]\(\s*' + re.escape(matched) + r'\s*\)', ctx, flags=re.IGNORECASE):
                return matched
            tail = matched.rstrip('/').split('/')[-1]
            if '/texture-packs/' in matched:
                return 'https://modrinth.com/resourcepack/' + tail
            else:
                return 'https://modrinth.com/mod/' + tail
        return pattern.sub(repl, content)
    else:
        pattern = re.compile(r'https?://(?:www\.)?modrinth\.com/(?:mod|resourcepack)/[A-Za-z0-9_\-./%]+')
        def repl(m):
            start = m.start()
            end = m.end()
            ctx_start = max(0, start - 200)
            ...
            if re.search(r'!\[[^\]]*\]\(\s*' + re.escape(matched) + r'\s*\)', ctx, flags=re.IGNORECASE):
                return matched
            tail = matched.rstrip('/').split('/')[-1]
            if '/resourcepack/' in matched:
                return 'https://www.curseforge.com/minecraft/texture-packs/' + tail
            else:
                return 'https://www.curseforge.com/minecraft/mc-mods/' + tail
        return pattern.sub(repl, content)
